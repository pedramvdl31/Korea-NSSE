...
class Job extends Model
{
	...
	static public function deleteContent($path){
		try{
			$iterator = new DirectoryIterator($path);
			foreach ( $iterator as $fileinfo ) {
			  if($fileinfo->isDot())continue;
			  if($fileinfo->isDir()){
			    if(Job::deleteContent($fileinfo->getPathname()))
	...
	//TRIM FOR SAVING FILES
	static public function FormatFileNames($d){
	...
	static public function GetFileInfo($df){
		$o_ar=['exten'=>'','or_name'=>''];
		if (isset($df)) {
			$o_ar['exten'] = pathinfo($df, PATHINFO_EXTENSION);
			$o_ar['or_name'] = pathinfo($df, PATHINFO_FILENAME);
		}
		return $o_ar;
	}
	...
	static public  function zipData($source, $destination) {
    ...
    //ZIP ALL AND ADD TO THE ZIP FOLDER WITH THE SAME NAME AS THE PARENT FOLDER ZIP CHRAT TO CHART.ZIP
	static public function zipDir($sourcePath, $outZipPath) 
	{ 
		$sp = DIRECTORY_SEPARATOR;
		$pathInfo = pathInfo($sourcePath); 
		$parentPath = $pathInfo['dirname'].$sp; 
		...
		Job::folderToZip($sourcePath, $z, strlen("$parentPath/")); 
		$z->close(); 
	} 
	    ...
	    while (false !== $f = readdir($handle)) { 
	      if ($f != '.' && $f != '..') { 
	        ...
	        if (is_file($filePath)) { 
	          ...
	          Job::folderToZip($filePath, $zipFile, $exclusiveLength); 
	        } 
	      } 
	    } 
	    closedir($handle); 
	...
	static public function dump2($results) {
		if(isset($results)) {
			echo '<style type="text/css">.main-sidebar{display:none}</style><pre class="job-dump">';
			print_r($results);
			echo '</pre>';
		}
	...
	static public function PrepareExcelTable($edata) {
		$html = '';

		// if (isset($edata)) {
		// 	Job::dump($edata);
		// 	foreach ($edata as $ek => $ev) {
		// 		Job::dump('**********');
		// 		Job::dump($ek);
		// 		foreach ($ev as $evk => $evv) {
		// 			Job::dump('-------------');
		// 			Job::dump($evk);
		// 			Job::dump($evv);
		// 			Job::dump('-------------');
